...
def plot_population_pyramid(
    ...
    *,
    ...
    # X-axis corrected: maintain visual shift, true scale
    max_pop = max(abs(males).max(), females.max())
    exp = np.floor(np.log10(max_pop))
    step = next((s * 10**exp for s in [5, 2, 1] if max_pop / (s * 10**exp) >= 4), 10**exp)
    ticks = np.arange(0, int(max_pop) + step, step)
    pos_ticks, neg_ticks = ticks + shift, -(ticks + shift)[::-1]
    all_ticks = np.unique(np.concatenate([neg_ticks, pos_ticks]))
    ax.set_xticks(all_ticks)
    
    # Tick labels: 0 starts where bars begin, show true population counts
    ax.xaxis.set_major_formatter(plt.FuncFormatter(
        lambda x, _: '0' if abs(x) == shift
        else (f'{int(abs(x) - shift)}' if (abs(x) - shift) % step == 0 and abs(x) > shift else '')
    ))
    pad = step * 0.2
    ax.set_xlim(-max(ticks) - shift - pad, max(ticks) + shift + pad)
    ax.set_xlabel("Population", labelpad=10, fontsize=10.5)
