...
sched_preventive = AppointmentScheduler(
    ...
    booking_horizon=35,      # 5-week forward window
...
2. **Future slot projection** – Illustrates extended forward availability within the five-week horizon.  
...
**Interpretation:**  
This configuration produces a flatter utilization pattern, where upcoming slots extend several weeks into the future but are only partially filled. The moderate fill rate keeps daily load manageable while maintaining accessibility — characteristic of preventive or low-urgency services.
